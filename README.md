...
## Дополнительные скрипты WordPress

### Оптимизация WordPress
После установки и начальной настройки WordPress рекомендуется запустить скрипт оптимизации:

```bash
sudo ./setup-files/wp-optimize.sh
```

Этот скрипт устанавливает полезные плагины для оптимизации производительности и безопасности, в том числе:
- W3 Total Cache - кеширование
- WP-Optimize - очистка базы данных и оптимизация
- Smush - оптимизация изображений
- Autoptimize - оптимизация CSS и JavaScript

Также скрипт устанавливает дополнительные полезные плагины: Classic Editor, Wordfence Security и UpdraftPlus.

### Резервное копирование WordPress
Для создания резервной копии WordPress (включая файлы и базу данных):

```bash
sudo ./setup-files/wp-backup.sh
```

Резервные копии сохраняются в директории `/opt/backups/wordpress/` и включают дамп базы данных (SQL) и архив всех файлов (tar.gz). Скрипт автоматически удаляет старые резервные копии, сохраняя только 5 последних.

Для автоматического резервного копирования рекомендуется добавить скрипт в cron:

```bash
sudo crontab -e
# Добавьте строку:
0 4 * * * /home/$(whoami)/cloud-local-n8n-flowise/setup-files/wp-backup.sh
```

Это создаст ежедневные резервные копии в 4:00 утра.
