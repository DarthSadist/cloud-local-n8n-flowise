...
echo "\n🔄 Подготовка к запуску Flowise..."

# Проверяем наличие тома flowise_data
if ! sudo docker volume inspect flowise_data &> /dev/null; then
  echo "⚠️ Том flowise_data не найден, создаем..."
  sudo docker volume create flowise_data
  if [ $? -ne 0 ]; then
    echo "❌ Не удалось создать том flowise_data" >&2
    exit 1
  fi
fi

# Для обратной совместимости создаем директорию /opt/flowise
echo "📂 Создание директории /opt/flowise для обратной совместимости..."
sudo mkdir -p /opt/flowise
sudo chmod 777 /opt/flowise
echo "✅ Директория /opt/flowise готова"

# Проверяем наличие образа Flowise
echo "📦 Проверка образа Flowise..."
sudo docker pull flowiseai/flowise:latest
if [ $? -ne 0 ]; then
  echo "⚠️ Предупреждение: Не удалось загрузить образ Flowise, проверьте подключение к интернету" >&2
else
  echo "✅ Образ Flowise успешно загружен"
fi

# Останавливаем существующий контейнер Flowise, если он существует
echo "🔄 Останавливаем существующий контейнер Flowise, если он существует..."
sudo docker stop flowise 2>/dev/null || true
sudo docker rm flowise 2>/dev/null || true
...
if [ $? -eq 0 ]; then
  ...
  # Даем Flowise время на инициализацию
  echo "⏳ Ждем 15 секунд, пока Flowise инициализируется..."
  sleep 15
  
  # Проверяем статус Flowise
  if sudo docker ps | grep -q "flowise"; then
    echo "✅ Flowise работает нормально"
    echo "📝 Последние логи Flowise:"
    sudo docker logs flowise --tail 5
  else
    echo "❌ Ошибка: Flowise не запущен после инициализации" >&2
    echo "📋 Проверьте логи для диагностики:"
    sudo docker logs flowise 2>/dev/null || echo "Логи недоступны"
  fi
else
  ((failed_services++))
  echo "❌ Критическая ошибка: Не удалось запустить Flowise стек" >&2
  echo "📋 Проверьте логи для диагностики:"
  sudo docker logs flowise --tail 20 2>/dev/null || echo "Логи недоступны"
  exit 1
fi
