...
# Проверяем наличие директории /opt/flowise
echo "📂 Проверка директории /opt/flowise..."
...
sudo docker pull flowiseai/flowise:1.3.4
if [ $? -ne 0 ]; then
  echo "⚠️ Предупреждение: Не удалось загрузить образ Flowise:1.3.4, будет использована последняя версия" >&2
else
  echo "✅ Образ Flowise:1.3.4 успешно загружен"
fi
